...
function Cart() {
  ...
  useEffect(() => {
    const fetch = async () => {
      const resp = await axios.get(process.env.BASE_URL + "/api/cart/get-user-cart", { headers })
  ...
  const deleteItem = async (bookid) => {
    const resp = await axios.put(process.env.BASE_URL + `/api/cart/remove-from-cart/${bookid}`, {}, { headers });
  ...
  const placeOrder = async () => {
    ...
    try {
      const resp = await axios.post(process.env.BASE_URL + `/api/order/place-order`, { order: Cart }, { headers });
