...
export const ViewBookDetails = () => {
  ...
  const handleFavourite = async () => {
    const resp = await axios.put(process.env.BASE_URL + "/api/favourite/add-book-to-favourite", {}, { headers });
    alert(resp.data.message);
  }

  const handleCart = async () => {
    const cart = await axios.put(process.env.BASE_URL + "/api/cart/add-book-to-cart", {}, { headers });
    alert(cart.data.message);
  }

  const deleteBook = async () => {
    const resp = await axios.delete(process.env.BASE_URL + "/api/book/delete-book", { headers });
    alert(resp.data.message);
    navigate('/all-books');
  }
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const resp = await axios.get(process.env.BASE_URL + `/api/book/get-book-by-id/${id}`);
